...
### 04. Point-Free ###
  ...
  *** Equational Reasoning ***        
    ...
    ->Equational Reasoning allows us to specify that some function is equationally similar to another one in terms
      of the shape (input and output signature) of them.
*/
...
*/
    ...
    */
    ...
    addn(
      [three, nine, constant(12), constant(20), constant(6)]
    )

    var numbers = [12, 4, 8, 9, 45, 5, 33, 8, 6, 2, 75, 10, 12, 11, 4, 7, 7]
...
/*
  ...
  *** Fusion ***
    ...
    ->The action of composing multiple functions instead of chaining list operators is called fusion.

    ->It is very common to end up with a large list of list operators chained one after the other in applications following FP best practices. Even though reaching escenarios like this
      represent a good outcome, especially when all the mapper, predicate and reducer functions follow single responsability principles, there's a downside in this approach, significantly enough
      when dealing with more large data structures.
      
      * Every single time a list operator is invoke, our program not only will iteratively handle each item in the list into the mapper/predicate/reducer function, it also is going to be creating
        intermediate data structures which are going to be disposed after every new operator call. This constitute a garbage collection overhead in addition to the looping effort the program
        has to perform, i.e. a performance hit in our app. 

      * Functional programmers instead of having long chain of method calls, will prefer to compose those operator's functions taking advantage of the fact that these function (mapper functions for
        example) has the same shape and they can be compose together associatively based on equational reasoning principle.
      
      * Composing function calls aids to explicitly indicate the reader what's the data flow in our app easily.

      * "Using functional principles in our code is what gives us these sort of guarantees that essentially avoid entire classes of problems that can happen... 
        We don't want to make easy to find bugs if we can make easy to dont have bugs in the first place"
*/
      ...
      // Instead, applying the fusion approach:
      list
        .map(
          compose(div3,mul2,add1)
        );      
/*
    ->Functional Programmers don't like to call methods on values, because that is a object-oriented approach where the method gets the value it is going to work upon implicitly through the this keyword.
